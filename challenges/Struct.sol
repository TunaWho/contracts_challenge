...
contract StructExamples {
    ...
    function get(
        ...
        require(_index >= 0, "Index cannot be less than zero");
        model = cars[_index].model;
        year = cars[_index].year;
        owner = cars[_index].owner;
    }

    function transfer(uint _index, address _owner) external {
        require(_index >= 0, "Index cannot be less than zero");
        require(_owner != address(0), "Zero address");
        Car storage newOwner = cars[_index];
        newOwner.owner = _owner;
    }
}
